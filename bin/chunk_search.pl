...
{			   ###########################################
   ...
   if (! $new) {                # old way
      my @chunk_hashes = ();
      my ($ids_array, $id_sequence, $minL, $maxL) = fasta_string_to_hash($fasta1_string);
      ...
      if (defined $other_fasta  and  -f $other_fasta) {
         ...
         my ($ids_array, $other_id_sequence, $ominL, $omaxL) = fasta_string_to_hash($other_fasta_string);
      ...
      if (defined $n_chunks) {
         ...
         push @chunk_set_objects, $a_chunk_set;
         $n_chunks -= $a_chunk_set->n_chunks();
         print STDERR "A: $n_chunk_sets $n_chunks: ", $a_chunk_set->n_chunks(), "\n";
         for (2..$n_chunk_sets) {
         ...
         push @chunk_set_objects, ChunkSet->new({index_id => $ids_array, seqid_seq => $id_sequence, chunk_specifiers => chunk_indices($indices, $chunk_size)});
         print STDERR "B: $n_chunk_sets ", $n_chunks // 'undef', "\n";
         for (2..$n_chunk_sets) {
      ...
      if (defined $other_fasta  and  -f $other_fasta) {
         my $other_fasta_string = TomfyMisc::fasta2seqon1line(file_to_string($other_fasta));
         my ($other_ids_array, $other_id_sequence, $ominL, $omaxL) = fasta_string_to_hash($other_fasta_string);
         ...
         while (my ($other_id, $other_seq) = each %$other_id_sequence) {
            ...
            for my $chunk_set_obj (@chunk_set_objects) {
      ...
      $t1 = gettimeofday();
      my ($ids_array, $id_sequence, $minL, $maxL) = fasta_string_to_hash($fasta1_string);
      my $n_sequences = scalar @$ids_array;
      my $sequence_length = ($minL == $maxL)? $minL : die "Sequence lengths not all equal!\n";
      my $chunk_set_object = ChunkSet->new({index_id => $ids_array, seqid_seq => $id_sequence, chunk_size => $chunk_size, sequence_length => $sequence_length, n_chunks => $n_chunks});
      ...
      if (defined $other_fasta  and  -f $other_fasta) {
         ...
         while (my ($other_id, $other_seq) = each %$other_id_sequence) {
            ...
            if (0) {
               ...
               @best_matchids = (sort {$otherid_matchcount->{$b} <=> $otherid_matchcount->{$a}} @best_matchids);
               @best_matchids = @best_matchids[0..$n_keep-1] if($n_keep < scalar @best_matchids);
               @best_matchids = map([$_, $otherid_matchcount->{$_}], @best_matchids);
               $oid_bestmatchids{$other_id} = \@best_matchids;
            } else {
         ...
         #########################################################################################
         print $output_string;
         print "# construct data structure: ", $t2-$t1, "   candidates; matches_count: $matches_count  time: ", $t3-$t2, "   dists to candidates; count: $distance_calc_count  time: ", $t4-$t3, "\n";
      }
...
sub fasta_string_to_hash{
   ...
   return (\@ids_array, $id_seq, $min_length, $max_length);
}
...
# void count_matches(int size, int* indices, int* matchingindex_counts){
#    int i=0;
#    for(i=0; i< size; i++){
