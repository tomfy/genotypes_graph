...
{			   ###########################################
  ...
  #    while (my($oid, $bestids) = each %oid_bestmatchids) {
	...
	print "$oid     ";
	for (@$bestids) {
	  $id_dist{$_} = distance_C($id_sequence->{$_}, $other_id_sequence->{$oid})
	}
	my @dsorted_ids  = sort { $id_dist{$a} <=> $id_dist{$b} } keys %id_dist;
	#	print "A: ", $id_sequence->{$_}, "\n";
	#	print "B: $other_seq\n";
	for (@dsorted_ids) {
	  printf("%4d  %8.5f   ", $_, $id_dist{$_});
	}
	print "\n";
      }   $t4 = gettimeofday();
    ...
    my $id_sequence = fasta_string_to_hash($fasta1_string);
    $t0 = gettimeofday(); 
    my $randomized_indices = [0 .. (scalar keys %$id_sequence) - 1];
    #  randomize_array([0 .. (scalar keys %$id_sequence) - 1]);
    print STDERR join(", ", @$randomized_indices), "\n";
    my $chunk_set_obj = ChunkSet->new({seqid_seq => $id_sequence, chunk_specifiers => chunk_indices($randomized_indices, $chunk_size)});
    $t1 = gettimeofday();
  #  print STDERR "# done storing sequences \n";
...
#	print "other seq: $other_seq \n";
	my $otherid_matchcount =  $chunk_set_obj->get_chunk_match_counts($other_seq);

	my @best_matchids = (sort {$otherid_matchcount->{$b} <=> $otherid_matchcount->{$a}} keys %$otherid_matchcount); # [0..$n_keep-1];
	@best_matchids = @best_matchids[0..$n_keep-1] if($n_keep < scalar @best_matchids);
#	   print STDERR "WWW: ", join(", ", @best_matchids), "\n";
...
#  print "Indices: ", join(", ", @$indices), "\n";
  ...
  while (scalar @$indices >= $chunk_size) {
    ...
    push @chunk_specs, join("_", splice( @$indices, 0, $chunk_size )); # @chunk_indices);
  }
